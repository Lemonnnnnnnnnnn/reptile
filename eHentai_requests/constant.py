# idm相关
idm_path = r'C:\Program Files (x86)\Internet Download Manager\IDMan.exe'
idm_name = 'IDMan.exe'
# aria2相关设置
aria2_path = r'D:\aria2\HideRun.vbs'
aria2_name = 'aria2c.exe'
# mongodb相关设置
mongodb_path = r'mongodb://localhost:27017/'
mongodb_db_name = 'ehentai'
mongodb_collection_name = 'album_item'
# 代理相关设置
proxy = 'http://localhost:7890'
user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Safari/537.36'
clash_name_list = ["台湾 01|DNS解锁节点|广移|1.0", "台湾 02|HiNet|青移|1.0", "台湾 03|HiNet|珠移|0.8", "新加坡 01|DOcean|广移|1.0",
                   "新加坡 02|Sakura|五邑|1.0", "新加坡 03|Azure|湘徐|1.0", "新加坡 04|DOcean|珠移|1.0", "日本 01|Linode|广移|1.0",
                   "日本 02|indigo|徐联|1.0", "日本 04|Linode|五邑|1.0", "日本 05|NHN|湘联|1.0", "美国 01|Seattle|五邑|1.0",
                   "美国 03|Inap|广移|1.0", "英国 01|BBC|珠移|0.8", "这个是俄罗斯节点", "这个是印度节点", "这个是土耳其节点", "这个是阿根廷节点", "这个是马来西亚节点",
                   "韩国 01|Azure|五邑1.0", "韩国 02|Azure|徐联|1.0", "韩国 03|Azure|湘联|1.0", "香港 00|Azure|湘联|1.0",
                   "香港 01|HGC|广移|1.0",
                   "香港 02|HKT|广移|隧道测试|1.0", "香港 03|HKT|湘联|1.0", "香港 04|Azure|青移|1.0", "香港 05|AGA|电信|0.2",
                   "香港 06|AGA|电信|0.2",
                   "香港 07|AGA|电信|0.2", "香港 08|HK-BGP|宁波|1.0"]
clash_port = '9090'
clash_url = 'http://localhost:' + clash_port + '/proxies/Proxy'
# 爬虫相关设置
# 爬取封面
crawl_start_url = 'https://e-hentai.org/?f_search=chinese'
crawl_custom = 'https://e-hentai.org/?f_search=kazuhiro+chinese'
face_download_url = r'D:\technology\python\reptile\eHentai_requests\django\www\public'
crawl_num = 5

# 图片存储路径
pic_download_url = r'E:\18x\pic'
